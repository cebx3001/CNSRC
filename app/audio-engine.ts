...
const SCENE_PROFILES: Record<AudioScene, SceneProfile> = {
  hero: { frequency: 46, air: 0.007, shimmer: 0.004, resonance: 0.7, gesture: "material" },
  sum: { frequency: 52, air: 0.007, shimmer: 0.004, resonance: 1.2, gesture: "integrated" },
  b2w: { frequency: 62, air: 0.006, shimmer: 0.003, resonance: 2.4, gesture: "signal" },
  monarca: { frequency: 43, air: 0.008, shimmer: 0.004, resonance: 1.1, gesture: "physical" },
  brown: { frequency: 55, air: 0.009, shimmer: 0.005, resonance: 4.2, gesture: "spatial" },
  tridifect: { frequency: 38, air: 0.007, shimmer: 0.003, resonance: 1.8, gesture: "material" },
  convergence: { frequency: 48, air: 0.009, shimmer: 0.005, resonance: 3.4, gesture: "integrated" },
  capacity: { frequency: 44, air: 0.007, shimmer: 0.003, resonance: 1.4, gesture: "signal" },
  "project-mall": { frequency: 41, air: 0.008, shimmer: 0.004, resonance: 2.2, gesture: "project" },
  "project-campaign": { frequency: 47, air: 0.009, shimmer: 0.005, resonance: 1.7, gesture: "project" },
  "project-corporate": { frequency: 39, air: 0.008, shimmer: 0.004, resonance: 3.1, gesture: "project" },
  finale: { frequency: 36, air: 0.006, shimmer: 0.003, resonance: 3.8, gesture: "finale" },
};
...
export class CnsrcAudioEngine {
  ...
  async start() {
    ...
    this.master.gain.linearRampToValueAtTime(0.48, now + 0.55);
    this.applyProfile(this.currentScene, 0.7);

    if (this.bed) this.bed.gain.setTargetAtTime(0.055, now, 0.45);
  ...
  setMuted(muted: boolean) {
    ...
    this.master.gain.cancelScheduledValues(now);
    this.master.gain.setValueAtTime(this.master.gain.value, now);
    this.master.gain.linearRampToValueAtTime(muted ? 0 : 0.48, now + (muted ? 0.22 : 0.5));
  ...
  setSceneState(scene: AudioScene, progress: number, velocity: number, density: number) {
    ...
    this.air.gain.setTargetAtTime(
      profile.air * (0.55 + shapedDensity * 0.18) * (0.86 + calm * 0.14),
      now,
      0.5,
    );

    this.bed.gain.setTargetAtTime(
      0.05 + shapedDensity * 0.014 + arc * 0.006 - speed * 0.004,
      now,
      0.6,
    );
    ...
    if (cueIndex >= 0 && performance.now() - this.lastCueAt > 180) this.triggerCue("text", cueIndex);
  ...
  enterScene(scene: AudioScene) {
    ...
    const profile = SCENE_PROFILES[scene];
    this.triggerCue("section");
    this.triggerGesture(profile.gesture, 0.13);
  }
  ...
  private createGraph() {
    ...
    airNoise.buffer = this.createAmbientNoise(context, 5, 0.18);
    airNoise.loop = true;
    const airFilter = context.createBiquadFilter();
    airFilter.type = "bandpass";
    airFilter.frequency.value = 860;
    ...
    shimmerNoise.buffer = this.createAmbientNoise(context, 5, 0.42);
  ...
  private triggerCue(kind: "text" | "section", index = 0) {
    ...
    const base = clamp(profile.frequency * 12.5, 540, 780);
    const frequency = kind === "section" ? Math.max(480, base - 70) : Math.min(930, base + pattern[index % pattern.length]);
