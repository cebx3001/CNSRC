      ...
      {children.split(" ").map((word, wordIndex, words) => (
        <span className="word-unit" aria-hidden="true" key={`${word}-${wordIndex}`}>
          {Array.from(word).map((character, characterIndex) => (
            <span className="char-wrap" key={`${character}-${characterIndex}`}><span className="char">{character}</span></span>
          ))}
...
export default function Home() {
    ...
    const context = gsap.context(() => {
      ...
      if (hero) {
        ...
        rows.forEach((row, rowIndex) => {
          ...
          heroTimeline.fromTo(chars, { rotateX: rowIndex === 0 ? -20 : (rowIndex % 2 ? 86 : -86), rotateY: rowIndex % 2 ? -18 : 18, scaleY: rowIndex === 0 ? 0.86 : 0.15, opacity: rowIndex === 0 ? 0.72 : 0.06 }, { rotateX: 0, rotateY: 0, scaleY: 1, opacity: 1, duration: 0.56, stagger: { each: 0.022, from: rowIndex === 2 ? "center" : "start" }, ease: "power3.out" }, beat);
          if (rowIndex < rows.length - 1) heroTimeline.to(chars, { rotateX: 78, scaleY: 0.22, opacity: 0.04, duration: 0.38, stagger: { each: 0.009, from: "edges" }, ease: "power2.in" }, beat + 0.7);
        });
        heroTimeline.fromTo(".hero-deck", { clipPath: "inset(0 100% 0 0)" }, { clipPath: "inset(0 0% 0 0)", duration: 0.7 }, 5.25).fromTo(".hero-rule", { scaleX: 0 }, { scaleX: 1, duration: 1.2, ease: "none" }, 0);
      }
      ...
      gsap.utils.toArray<HTMLElement>("[data-kinetic]").forEach((section, sectionIndex) => {
        ...
        timeline.fromTo(chars, { rotateY: sectionIndex % 2 ? 88 : -88, scaleX: 0.2, opacity: 0.12 }, { rotateY: 0, scaleX: 1, opacity: 1, stagger: { each: 0.012, from: sectionIndex % 3 === 0 ? "center" : "start" }, duration: 0.8, ease: "power3.out" }, 0);
        if (mediaBands.length) {
          timeline.fromTo(mediaBands, { clipPath: "inset(0 50% 0 50%)", z: -180, filter: "grayscale(1) contrast(1.25)" }, { clipPath: "inset(0 0% 0 0%)", z: 0, filter: "grayscale(.2) contrast(1.08)", stagger: 0.08, duration: 1.1, ease: "power2.inOut" }, 0.08);
          timeline.to(mediaBands, { clipPath: "inset(48% 0 48% 0)", z: 120, stagger: { each: 0.05, from: "edges" }, duration: 0.75 }, 1.55);
        }
        if (rails.length) timeline.fromTo(rails, { "--rail-scale": 0 }, { "--rail-scale": 1, duration: 1.2, ease: "none" }, 0.1);
  ...
  return (
    <main ref={root} className="site-shell">
      ...
      <section className="company-flow scene-dark">{companies.map((company) => <article className={`company-scene kinetic-scene ${company.rich ? "is-rich" : ""}`} data-kinetic key={company.name}><div className="sticky-stage company-stage"><div className="company-media" aria-hidden="true">{[0,1,2,3,4].map((band) => <div className={`media-band band-${band+1}`} key={band}><img src={company.image} alt="" style={{ objectPosition: company.position }} /></div>)}</div><div className="scene-index">CORRIENTE {company.index} <span>{company.index}—04</span></div><h2 className="company-name"><KineticText>{company.name}</KineticText></h2><div className="company-copy"><h3>{company.role}</h3><p>{company.description}</p></div><div className="term-rail"><span>{company.terms}</span><span aria-hidden="true">{company.terms}</span></div></div></article>)}</section>
